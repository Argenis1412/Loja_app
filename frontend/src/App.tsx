...
import {
  ...
  type PagamentoSimulacao,
} from './components/ConfirmacaoPagamento';
...
export default function App() {
  ...
  const [dadosPagamento, setDadosPagamento] =
    useState<CriarPagamentoRequest | null>(null);
  const [simulacao, setSimulacao] = useState<PagamentoSimulacao | undefined>(
    undefined,
  );
  ...
  return (
    <div className="min-h-screen bg-zinc-50 p-4 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100 transition-colors duration-300">
      ...
      <div className="mx-auto max-w-2xl py-10">
        ...
        {tela === 'recibo' && recibo && (
          <Recibo pagamento={recibo} onNovoPagamento={handleNovoPagamento} />
        )}
      </div>
    </div>
  );
