...
def test_idempotency_key_prevents_duplicate_creation():
    ...
    assert response2.status_code == 201 # Retorna 201 ou 200 dependendo da implementação, mas o importante é o mesmo ID
...
def test_different_idempotency_keys_create_different_records():
    ...
    client.post("/api/v1/pagamentos/", json=payload, headers={"Idempotency-Key": "key-a"})
    client.post("/api/v1/pagamentos/", json=payload, headers={"Idempotency-Key": "key-b"})
